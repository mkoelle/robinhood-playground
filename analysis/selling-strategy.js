...
module.exports = async (Robinhood, daysBack = 5) => {
    ...
    const robinhoodTransactions = await loadAllRobinhoodTransactions(
        Robinhood, 
        convertDateToRhFormat(dailyTransactionDates[daysBack])
    );
    ...
    const analyzeDay = async date => {
        ...
        // console.log({ rhDate });
        const rhSells = robinhoodTransactions
            .filter(t => t.updated_at.includes(rhDate))
            .filter(t => t.side === 'sell');
        const formattedRobinhoodSells = rhSells.map(sellOrder => ({
            ticker: sellOrder.instrument.symbol,
            bid_price: Number(sellOrder.average_price),
            quantity: sellOrder.quantity,
        }));
        // console.log(JSON.stringify({rhSells, date}, null, 2));
        
        // console.log({fullPath, dailyTransactionDates, rhSells})
        ...
        const robinhoodSellsNotIncluded = formattedRobinhoodSells
        ...
        const combined = allSellsToday.map(sell => {
            const relatedBuy = associatedBuys.find(obj => obj.ticker === sell.ticker);
            return {
                ticker: sell.ticker,
                ...(relatedBuy && {
